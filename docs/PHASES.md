...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | Five exhaustion, abandoned-owner, cohort-authorization, publication-attempt and canonical-pair findings are remediated in one eighth local documentation commit on top of the seven earlier design commits and remain pending a fresh independent complete eight-commit review. The design defines transactional `supplier_import_dispatch_outbox`; one stable execution claim shared by both XML paths with `pending_dispatch`, `queued`, `processing` and immutable terminal states; pair-null orchestrated dispatch followed by owner-checked atomic allocation; early pair-bound legacy authorization; retained one-claim-per-ImportJob uniqueness plus a separately named three-column child FK index; dedicated `redis_supplier_import` / `supplier-imports` routing and worker at `retry_after=3900` while unrelated Redis work remains at `retry_after=1300`; `$tries=8` with no `retryUntil()`; immutable MySQL-UTC-plus-24-hour deadline and cumulative delivery budget; exact `3600 < 3900 < 4200 < 4320` timing; direct terminal closeout for exhausted delivery/deadline boundaries; recoverable-only `recovery_required` with an explicit terminal reconciler path; successful publication attempt eight and terminal failed/ambiguous attempt eight; separate raw-token live-owner and expired-tuple abandoned-owner APIs; immutable consistent-snapshot cohort authorization fields and hashed seed members before source work; exact source-only expansion without mutable membership rereads; canonical claim/outbox pairs; no importer replay after the non-repeatable staging boundary; atomic authoritative parent/history/evidence/claim finalization; exact `ascii`/`ascii_bin` hexadecimal checks; append-only authorization/cohort/generation/observation records; exact indexes; bounded streaming; deterministic V4 qualification/gaps; and 49 fine-grained rollout checkpoints. The dedicated worker adds no schedule. It adds no table, migration, config, Docker service, command, parser change, capture implementation, producer, backfill, import or operational evidence. |
