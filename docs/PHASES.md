...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | Three additional queued-transport findings are remediated in one ninth local documentation commit on top of the eight earlier design commits and remain pending a fresh independent complete nine-commit review. The unimplemented design now gives every `queued/published` substate a bounded outcome: `delivery_watchdog_at` is set from MySQL UTC plus exactly 4,320 seconds after acknowledged publication and selected through `ix_import_dispatch_outbox_state_watchdog_id(state, delivery_watchdog_at, id)`; a due null-owner payload uses `dispatch_payload_unobserved` for same-key recovery or atomic terminal exhaustion; `ExpiredQueuedImportTerminalRepository::resolveExpiredQueuedOwnership()` uses a new supplier lock and complete expired-tuple CAS without the lost raw token; and the exact dry-run-first one-execution `suppliers:resolve-import-publication-mismatch` command uses `PublicationMismatchTerminalRepository::failPreProcessingMismatch()` with strict parent authority and idempotent rerun. The canonical crash matrix is 36 rows by 11 columns and the rollout matrix remains 49 checkpoints. All earlier transactional outbox, stable claim, pair-null/pair-bound allocation, queue isolation, `$tries=8` without `retryUntil()`, immutable 24-hour deadline, cumulative delivery budget, `3600 < 3900 < 4200 < 4320` timing, processing-replay prohibition, authoritative parent/evidence finalization, immutable cohort authorization, exact index/FK, bounded streaming and deterministic V4 contracts remain unchanged. Persistence remains local, unapproved, unimplemented and undeployed; no table, migration, config, Docker service, command, parser, capture implementation, producer, backfill, import or operational evidence exists. |
