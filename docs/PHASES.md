...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | The independently confirmed external-effect fencing findings are remediated locally and require a fresh aggregate review. A due null-owner watchdog proves only no durable processing progress and uses `dispatch_durable_progress_stalled`; it never claims nondelivery or no handler observation. The unimplemented 300-second monitor is domain-read-only but owns exact generation-bound heartbeat and byte-canonical privacy-safe alert-intent coordination; a successful cycle and sink acknowledgement no older than 600 seconds plus an independent generation-bound 60-second observer are required, while `stale`, `failed` and `unknown` reject capture, protected-generation start, authorization issuance and recovery start. Alert readiness now requires a concrete adapter proving native external generation fencing or durable provider idempotency under the stable `alert_identity`; unsupported providers fail closed, attempt-eight uncertainty remains `delivery_outcome_unknown_exhausted` at count eight with no attempt nine or additional logical effect, and no provider is selected here. Five mutating recovery actions use authenticated active-Super-Admin issuance, complete tuple/fingerprint binding, protected stdin nonce and action-compatible append-only results. `republish_same_key` can only republish, fail publication or action-stop without terminalizing; every physical Redis effect requires a committed counter-incrementing generation/token reservation plus atomic Redis-side monotonic fence and one-use publication, while local call-boundary CAS is audit only. Unknown classification retires the Redis fence first, and stale A creates zero publish effects after successor B advances it. `recover_expired_queued_ownership` supplies the legal first complete-owner CAS and cannot clear first. `expected_state_fingerprint_v2` has exactly 20 ordered fields including `claimed_at`, exact canonical bytes and a synthetic vector; it is distinct from the 16-field resume fingerprint. The canonical inventories contain ten tables and 22 cryptographic/digest identities. Alert identity uses one exact NUL-delimited domain, six-key canonical JSON and two synthetic vectors; monitor/observer/alert tables have exact MySQL keys/checks/FK and generation CAS. Dispatch payload bytes and its existing vectors remain canonical. The protocol matrix is 19 by 3, crash matrix 66 by 11 and rollout matrix 103 by 8; their refined rows explicitly cover Redis/provider fencing, suspended stale workers and external-effect counters without artificially changing dimensions. All five PR chains separately establish/implement, validate, independently review, remediate or record not-required, obtain fresh PASS, authorize/push, verify remote SHA, create/verify Draft PR, run CI/review and authorize/perform merge. Operational rollback is forward-only and evidence-preserving; destructive down is allowed only for the exact confirmed empty local/testing schema predicate. Phase I's canonical schema is implemented, merged through PR #212, CI-verified and deployed to staging inactive. Phase II's guarded models and frozen canonical byte contracts are implemented, merged through PR #213, CI-verified and deployed to staging uncalled. Phase III persistence remains unimplemented and not implementation-authorized: `PH3-RDY-001` and `PH3-RDY-004` are closed, while `PH3-RDY-002` requires a separately approved immutable claim/source binding and `PH3-RDY-003` requires approved importer/source maxima and derived spool/sort/insert/transaction bounds. No Redis Function, provider adapter, config activation, Docker worker, monitor, command, parser, capture implementation, producer, backfill, import or operational evidence exists. |

### Phase 9C.6.5C.3D.1-PRE.A Rollout Checkpoints

The authoritative sequence is the
...
It gives separate rows to candidate/implementation, validation, independent
review, remediation-or-not-required, fresh independent PASS, authorization, push,
remote-branch verification, Draft PR, PR base/head verification,
CI, merge, deployment, verification, enablement, import, producer, candidate,
preview and closeout action. No row combines review/merge, merge/deployment,
candidate preparation/approval, approval/preview or result review/closeout. A
failed checkpoint blocks every later checkpoint.
...
| Phase | Name | Status |
| --- | --- | --- |
| Phase 9C.6.5C.3D.1 | Controlled Operational Offer Lifecycle Preview | C3D.1 remains blocked by `BLOCKED_HISTORICAL_SOURCE_CONTRACT_REQUIRED`. V4 remains authoritative and V1-V3 remain historical. Mutable staging cannot be backfilled; no outbox/claim/recovery/monitor schema, dedicated queue/worker, parser, capture/idempotency implementation or producer exists. The local unapproved design requires dedicated `redis_supplier_import` / `supplier-imports`, `$tries=8` without `retryUntil()`, non-resetting 24-hour deadline and cumulative delivery budget, exact `3600 < 3900 < 4200 < 4320` timing, and a 4,320-second watchdog that never infers nondelivery. Monitor/sink/observer health continuously gates capture and recovery; attempt-eight alert uncertainty is durable and blocks automatic retry. All five mutating recovery actions use authenticated, nonce-proven, complete-tuple authorization and deterministic action-specific pre-start/post-start validation; each physical Redis publication has a committed one-use attempt reservation, and dispatch payload/monitor-alert hashing remain byte-canonical. One non-null orchestrated claim per run, byte-exact immutable path, exact parent shape, outbox-only `recovery_required`, published-only processing/finalization and the 66-by-11 crash matrix remain required. The phase follows exactly 103 checkpoints, including separate candidate/implementation, validation, independent review, remediation/fresh PASS, push authorization, push, remote-SHA verification, Draft PR, PR base/head verification, CI and PR review in all five chains; checkpoint 64 still requires one baseline plus three comparable absences and at least 48 hours from absence 1, and no operational preview is permitted before checkpoint 87. Operational rollback is forward-only and preserves all durable evidence. No evidence candidate exists. Every review, PR, merge, deployment, enablement, import, producer, candidate, preview and closeout action requires its own checkpoint. Supplier #3 remains unselected and unstarted. |
