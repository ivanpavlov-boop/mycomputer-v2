...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | Design PR #211 is merged. Phase I's canonical schema is implemented through PR #212, CI-verified, deployed to staging, present, and behaviorally inactive. Phase II's guarded models and frozen canonical byte contracts are implemented through PR #213, CI-verified, deployed to staging, and uncalled by supplier runtime. Phase III remains unimplemented, blocked, and not implementation-authorized. Architecture now closes immutable candidate provenance (`PH3-RDY-001`) through an immutable source profile, append-only execution, stable supplier/feed/SKU identity head and append-only revision chain, and closes durable claim source binding (`PH3-RDY-002`) through the exact five-field tuple and composite generation FK. All nine production operational bounds remain `NOT SPECIFIED`, so `PH3-RDY-003` is the single remaining architecture blocker; `PH3-RDY-004` remains closed. Claim source binding does not replace candidate provenance. The protocol matrix remains 19 by 3, crash matrix 66 by 11, rollout matrix 103 by 8, and frozen canonical inventory remains ten tables with 22 identities; eight new future identities are separately versioned. Operational rollback remains forward-only and evidence-preserving. No migration, Redis Function, provider adapter, config activation, Docker worker, monitor runtime, command, parser/capture implementation, producer, historical backfill, import, or operational evidence exists. |
