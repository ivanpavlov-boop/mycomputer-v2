...
It gives separate rows to every authorization, implementation, review, push,
...
| Phase | Name | Status |
| --- | --- | --- |
| Phase 9C.6.5C.3D.1 | Controlled Operational Offer Lifecycle Preview | C3D.1 remains blocked by `BLOCKED_HISTORICAL_SOURCE_CONTRACT_REQUIRED`. V4 remains authoritative and V1-V3 remain historical. Mutable staging cannot be backfilled; no outbox/claim/recovery/monitor schema, dedicated queue/worker, parser, capture/idempotency implementation or producer exists. The local unapproved design requires dedicated `redis_supplier_import` / `supplier-imports`, `$tries=8` without `retryUntil()`, non-resetting 24-hour deadline and cumulative delivery budget, exact `3600 < 3900 < 4200 < 4320` timing, and a 4,320-second watchdog that never infers nondelivery. Monitor/sink/observer health continuously gates capture and recovery. All five mutating recovery actions use authenticated, nonce-proven, complete-tuple authorization and deterministic action-specific pre-start/post-start validation; dispatch payload and monitor-alert hashing are byte-canonical. One non-null orchestrated claim per run, byte-exact immutable path, exact parent shape, outbox-only `recovery_required`, published-only processing/finalization and the 53-by-11 crash matrix remain required. The phase follows exactly 89 checkpoints, including separate push authorization, push, remote-SHA verification, Draft PR, PR base/head verification, CI and review in all five PR chains; checkpoint 56 still requires one baseline plus three comparable absences and at least 48 hours from absence 1, and no operational preview is permitted before checkpoint 76. Operational rollback is forward-only and preserves all durable evidence. No evidence candidate exists. Every review, PR, merge, deployment, enablement, import, producer, candidate, preview and closeout action requires its own checkpoint. Supplier #3 remains unselected and unstarted. |
