...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | The latest independent-review A1-A5 findings are remediated locally and require another fresh independent aggregate review. A due null-owner watchdog proves only no durable processing progress and uses `dispatch_durable_progress_stalled`; it never claims nondelivery or no handler observation. The unimplemented 300-second monitor is domain-read-only but owns exact generation-bound heartbeat and byte-canonical privacy-safe alert-intent coordination; a successful cycle and sink acknowledgement no older than 600 seconds plus an independent generation-bound 60-second observer are required, while `stale`, `failed` and `unknown` reject capture, protected-generation start, authorization issuance and recovery start. Alert attempt-eight uncertainty CASes to `delivery_outcome_unknown_exhausted` at count eight, is neither acknowledged nor permanently failed, cannot retry automatically or reach attempt nine, and keeps admission unhealthy. Five mutating recovery actions use authenticated active-Super-Admin issuance, complete tuple/fingerprint binding, protected stdin nonce and action-compatible append-only results. `republish_same_key` can only republish, fail publication or action-stop without terminalizing; every physical Redis call requires a committed counter-incrementing generation/token reservation and one-use call-boundary CAS, unresolved attempts consume their ordinal as `outcome_unknown`, stale generations lose, and a different terminal action requires new authorization. `recover_expired_queued_ownership` supplies the legal first complete-owner CAS and cannot clear first. `expected_state_fingerprint_v2` has exactly 20 ordered fields including `claimed_at`, exact canonical bytes and a synthetic vector; it is distinct from the 16-field resume fingerprint. The canonical inventories contain ten proposed tables and 22 cryptographic/digest identities. Alert identity uses one exact NUL-delimited domain, six-key canonical JSON and two synthetic vectors; monitor/observer/alert tables have exact MySQL keys/checks/FK and generation CAS. Dispatch payload bytes and its existing vectors remain canonical. The protocol matrix is 19 by 3, crash matrix 66 by 11 and rollout matrix 103 by 8; all five PR chains separately establish/implement, validate, independently review, remediate or record not-required, obtain fresh PASS, authorize/push, verify remote SHA, create/verify Draft PR, run CI/review and authorize/perform merge. Operational rollback is forward-only and evidence-preserving; destructive down is allowed only for the exact confirmed empty local/testing schema predicate. All prior outbox, stable claim, allocation, queue isolation, `$tries=8` without `retryUntil()`, immutable deadline/budget, `3600 < 3900 < 4200 < 4320` timing, replay prohibition, parent/evidence finalization, immutable cohort authorization, exact path/index/FK, bounded streaming and deterministic V4 contracts remain planned. Persistence remains local, unapproved, unimplemented and undeployed; no table, migration, config, Docker service, monitor, command, parser, capture implementation, producer, backfill, import or operational evidence exists. |
