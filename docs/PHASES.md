...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | Eight aggregate-review findings are remediated in a twelfth local documentation commit and remain pending a fresh independent complete twelve-commit review. A due null-owner watchdog proves only no durable processing progress and uses `dispatch_durable_progress_stalled`; it never claims nondelivery or no handler observation. The unimplemented 300-second monitor is domain-read-only but owns dedicated heartbeat and privacy-safe alert-intent coordination; a successful cycle and sink acknowledgement no older than 600 seconds plus an independent healthy 60-second observer are required, while `stale`, `failed` and `unknown` reject capture, protected-generation start, authorization issuance and recovery start. Every mutating recovery action is issued only through authenticated Filament for an active Super Admin, binds the complete action/operator/claim/outbox/key/parent tuple and server-computed pre-state fingerprint, expires after 900 seconds, accepts one protected stdin nonce, and records composite-bound append-only results. Same-key publication has one exact post-start resume fingerprint; publication mismatch and abandoned-processing apply have no prose-only exception. Dispatch payload bytes, domain-separated hash and two synthetic vectors are canonical. The protocol table remains 12 outcomes, crash matrix 36 by 11 and rollout matrix 53 by 8, with monitor implementation, sink/observer enablement and continuous health gating separated from capture. All prior outbox, stable claim, allocation, queue isolation, `$tries=8` without `retryUntil()`, immutable deadline/budget, `3600 < 3900 < 4200 < 4320` timing, replay prohibition, parent/evidence finalization, immutable cohort authorization, exact path/index/FK, bounded streaming and deterministic V4 contracts remain planned. Persistence remains local, unapproved, unimplemented and undeployed; no table, migration, config, Docker service, monitor, command, parser, capture implementation, producer, backfill, import or operational evidence exists. |
...
| Phase | Name | Status |
| --- | --- | --- |
| Phase 9C.6.5C.3D.1 | Controlled Operational Offer Lifecycle Preview | C3D.1 remains blocked by `BLOCKED_HISTORICAL_SOURCE_CONTRACT_REQUIRED`. V4 remains authoritative and V1-V3 remain historical. Mutable staging cannot be backfilled; no outbox/claim/recovery/monitor schema, dedicated queue/worker, parser, capture/idempotency implementation or producer exists. The local unapproved design requires dedicated `redis_supplier_import` / `supplier-imports`, `$tries=8` without `retryUntil()`, non-resetting 24-hour deadline and cumulative delivery budget, exact `3600 < 3900 < 4200 < 4320` timing, and a 4,320-second watchdog that never infers nondelivery. Monitor/sink/observer health continuously gates capture and recovery. All four mutating recovery domains use authenticated, nonce-proven, complete-tuple authorization and deterministic pre-start/post-start validation; dispatch payload hashing is byte-canonical. One non-null orchestrated claim per run, byte-exact immutable path, exact parent shape, outbox-only `recovery_required`, published-only processing/finalization and the 36-by-11 crash matrix remain required. The phase follows exactly 53 checkpoints, including separate monitor review/merge/disabled deployment/sink-observer enablement; checkpoint 28 still requires one baseline plus three comparable absences and at least 48 hours from absence 1, and no operational preview is permitted before checkpoint 44. No evidence candidate exists. Every review, PR, merge, deployment, enablement, import, producer, candidate, preview and closeout action requires its own checkpoint. Supplier #3 remains unselected and unstarted. |
