...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | Six recovery-authorization and monitor-boundary findings are remediated in an eleventh local documentation commit on top of the ten previously reviewed design commits and remain pending a fresh independent complete eleven-commit review. The unimplemented design distinguishes read-only detection from manually authorized mutation: `delivery_watchdog_at` is non-null only for the exact `queued/published` pair, no observation or monitor path refreshes it, the indexed scheduled monitor runs every 300 seconds without writes/jobs, warning begins when due, critical begins 1,800 seconds later and repeats every 900 seconds, and lack of operator action may remain nonterminal and critically alerted. Recovery requires one immutable exact-pair/action/canonical-state-fingerprint authorization expiring after 900 seconds, a 32-byte single-display nonce accepted only through stdin, and append-only ordered `started` plus terminal lifecycle result events. Same-key republication is forbidden after the operator objective and exact fail-closed terminalization remains the only authorized mutation. `uq_import_execution_claim_run` enforces one non-null orchestrated claim per run while permitting legacy nulls, while byte-exact immutable `execution_path` and `chk_import_execution_claim_path_parent` enforce the exact orchestrated/legacy parent shape. The canonical protocol table remains 12 outcomes, the crash matrix remains 36 rows by 11 columns and the rollout matrix now contains 53 checkpoints, including separate monitor implementation review, merge, disabled deployment and read-only schedule-enablement gates. All earlier transactional outbox, stable claim, allocation, queue isolation, `$tries=8` without `retryUntil()`, immutable deadline/budget, `3600 < 3900 < 4200 < 4320` timing, replay prohibition, parent/evidence finalization, immutable cohort authorization, exact index/FK, bounded streaming and deterministic V4 contracts remain unchanged. Persistence remains local, unapproved, unimplemented and undeployed; no table, migration, config, Docker service, monitor, command, parser, capture implementation, producer, backfill, import or operational evidence exists. |
...
It gives separate rows to every authorization, implementation, review, push/PR,
...
| Phase | Name | Status |
| --- | --- | --- |
| Phase 9C.6.5C.3D.1 | Controlled Operational Offer Lifecycle Preview | C3D.1 remains blocked by `BLOCKED_HISTORICAL_SOURCE_CONTRACT_REQUIRED`. V4 remains authoritative and V1-V3 remain historical. Mutable staging cannot be backfilled; no outbox/claim/recovery-authorization/schema migration, dedicated queue/worker, queue-timing change, watchdog monitor, expired-owner repository, mismatch command, streaming parser, capture/idempotency implementation or producer exists. The local unapproved design requires dedicated `redis_supplier_import` / `supplier-imports` routing at `retry_after=3900` while unrelated Redis work remains at `retry_after=1300`; `$tries=8` without `retryUntil()`; a non-resetting durable 24-hour deadline and cumulative delivery budget; exact `3600 < 3900 < 4200 < 4320` timeout/retry/DB-lease/Redis-TTL timing; a 4,320-second acknowledged-payload watchdog cleared outside `queued/published` and never refreshed by observation; read-only 300-second monitoring; immutable 900-second exact manual recovery authority bound to the canonical state fingerprint, one 32-byte single-display stdin nonce, and append-only ordered lifecycle result events; owner-independent expired queued-owner recovery; exact one-execution publication-mismatch terminal resolution; one non-null orchestrated claim per run with byte-exact immutable path and exact parent shape; canonical outbox-only `recovery_required`; processing/finalization gated on `outbox.state=published`; and the 36-row by 11-column parent-state crash matrix before implementation can be authorized. The dedicated worker adds no automatic recovery schedule. The phase follows the 53-checkpoint lifecycle, including separate monitor review, merge, disabled deployment and read-only schedule enablement; checkpoint 28 requires one immutable baseline plus three later comparable absences in one unchanged cohort epoch and at least 48 hours from absence 1, and no operational preview is permitted before checkpoint 44. No evidence candidate exists. An authorized cohort expansion uses its complete enrollment generation as a new baseline; unexpected drift freezes. Every review, PR, merge, deployment, enablement, import, producer, candidate, preview and closeout action requires its own checkpoint. Supplier #3 remains unselected and unstarted. |
