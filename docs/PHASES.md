...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | Design PR #211 is merged. Phase I's historical canonical ten-table schema is implemented through PR #212, CI-verified, deployed to staging, present, and behaviorally inactive. Phase II's guarded models and frozen canonical byte contracts are implemented through PR #213, CI-verified, deployed to staging, and uncalled by supplier runtime. Phase III remains unimplemented, blocked, and not implementation-authorized. The selected future architecture adds a dormant provenance prerequisite subphase with immutable profile/context/execution, stable supplier/feed/SKU identity heads, append-only revisions, guarded staging pointers, exact claim-source binding, bounded source capture and a ten-bound policy. All ten numeric production values remain `NOT SPECIFIED`; the exact current gate map is available only through the canonical reference above. Claim source binding does not replace candidate provenance. The protocol matrix remains 19 by 3, crash matrix 66 by 11, rollout matrix 103 by 8, and frozen Phase I/II inventory remains ten deployed tables with 22 identities; future identities remain separately versioned. Operational rollback remains forward-only and evidence-preserving. No provenance migration, Redis Function, provider adapter, config activation, Docker worker, monitor runtime, command, parser/capture implementation, producer, historical backfill, import, or operational evidence exists. |
