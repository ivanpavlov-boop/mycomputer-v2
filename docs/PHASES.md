...
| Phase 9C.6.5C.3D.1-PRE.A | Immutable Supplier Offer Snapshot Persistence Design | Four additional schema, transport-deadline, cohort-epoch and outbox-recovery review findings are remediated locally on top of the six earlier design commits and remain pending a fresh independent complete seven-commit review. The design now defines transactional `supplier_import_dispatch_outbox`; one stable execution claim shared by both XML paths with `pending_dispatch`, `queued`, `processing` and immutable terminal states; pair-null orchestrated dispatch followed by owner-checked atomic allocation; early pair-bound legacy authorization; retained one-claim-per-ImportJob uniqueness plus a separately named three-column child FK index; dedicated `redis_supplier_import` / `supplier-imports` routing and worker at `retry_after=3900` while unrelated Redis work remains at `retry_after=1300`; `$tries=8` with no `retryUntil()`; an immutable MySQL-UTC-plus-24-hour outbox deadline and non-resetting cumulative delivery budget; exact `3600 < 3900 < 4200 < 4320` job/retry/DB-lease/Redis-TTL timing; one MySQL UTC ownership-tuple CAS within a 60-second bootstrap; in-handle owner-proven exception closeout; transport-only newly deserialized `failed()`; canonical outbox-only `recovery_required`; fixed-order ownership/outbox transactions that require `published` before processing/finalization; separate queue-delivery, logical-processing and outbox-publication attempts; explicit canonical SupplierImportRun, ImportJob and ImportHistory crash states; no importer replay after the non-repeatable staging boundary; atomic authoritative parent/history/evidence/claim finalization; fail-closed abandoned-processing recovery; expected enrollment-generation baselines with `capture_cohort_changed` reserved for unexpected drift; exact `ascii`/`ascii_bin` hexadecimal checks; append-only cohort/generation/observation evidence; exact indexes; bounded streaming; deterministic V4 qualification/gaps; and 49 fine-grained rollout checkpoints. The dedicated worker adds no schedule. It adds no table, migration, config, Docker service, command, parser change, capture implementation, producer, backfill, import or operational evidence. |
...
| Phase | Name | Status |
| --- | --- | --- |
| Phase 9C.6.5C.3D.1 | Controlled Operational Offer Lifecycle Preview | C3D.1 remains blocked by `BLOCKED_HISTORICAL_SOURCE_CONTRACT_REQUIRED`. V4 remains authoritative and V1-V3 remain historical. Mutable staging cannot be backfilled; no outbox/claim/allocation/schema migration, dedicated queue/worker, queue-timing change, recovery command, streaming parser, capture/idempotency implementation or producer exists. The unapproved design requires dedicated `redis_supplier_import` / `supplier-imports` routing at `retry_after=3900` while unrelated Redis work remains at `retry_after=1300`; `$tries=8` without `retryUntil()`; a non-resetting durable 24-hour deadline and cumulative delivery budget; exact `3600 < 3900 < 4200 < 4320` timeout/retry/DB-lease/Redis-TTL timing; one MySQL UTC full-ownership-tuple CAS; in-handle owner-proven closeout; transport-only `failed()`; canonical outbox-only `recovery_required`; processing/finalization gated on `outbox.state=published`; exact outbox/ownership checks; and canonical parent-state recovery before implementation can be authorized. The dedicated worker adds no schedule. The phase follows the full 49-checkpoint lifecycle; checkpoint 24 requires one immutable baseline plus three later comparable absences in one unchanged cohort epoch and at least 48 hours from absence 1, and no operational preview is permitted before checkpoint 40. An authorized cohort expansion uses its complete enrollment generation as a new baseline; unexpected drift freezes. Every review, PR, merge, deployment, enablement, import, producer, candidate, preview and closeout action requires its own checkpoint. Supplier #3 remains unstarted. |
