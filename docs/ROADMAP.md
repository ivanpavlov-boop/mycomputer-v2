...
subphases. Commerce Phase 1C is proposed for persistent single-source storefront
state, error/loading UX, currency, multi-tab and browser coverage. Commerce Phase 1D
is proposed as the checkout, idempotency, payment, shipping, stock and release
gate. Phases 1C and 1D have not started.

Commerce Phase 1B is now split into controlled subphases. Phase 1B.1 is
merged, deployed and staging verified. It centralizes request-level Cart
session validation, ownership enforcement and anonymous-Cart claiming,
...
supplied eligible guest Cart is the login merge target. Paid items and bundles
retain stored prices; excessive quantity or conflicting coupons return a
generic `409` with a full rollback.

The manual `carts:expire-stale` command is preview-only unless `--apply` is
explicitly supplied. It is not scheduled and no Cart records or Cart content are
deleted. Pricing, stock feedback, promotion concurrency, recovery, checkout
...
CART-014 and CART-015. The Cart row serializes same-Cart mutation; paid and automatic-gift
...
redemptions and counters together. Checkout performs this consumption before
shipping, payment, stock, Cart conversion, rewards, jobs, email and events.

Recovery now locks and consumes one abandoned record at successful restore,
records a distinct `restored` state and links the restored Cart through a
unique `restored_cart_id`. Current authentication remains authoritative; a
token does not authenticate or assign the saved User. Existing populated or
historical target Carts remain unchanged and recovery returns a fresh Cart.
...
unchanged. CART-008 was partially remediated here; Phase 1D.2B completes its
local remediation.
CART-023 remains open, broader acceptance remains assigned to Phase 1D.3, and
the Nginx commerce gate is unchanged.

Commerce Phase 1D.2B is complete locally only. Direct authenticated ownership
or a dedicated guest retry capability authorizes the two narrow endpoints.
Only hashes of client keys and guest capabilities are persisted. One
transactional service locks the Order, rechecks authorization, applies the
explicit retry policy and returns or creates at most one safe payment result.
COD, bank transfer and leasing are not retryable. Card remains disabled by
default, the production provider remains non-operational, and no external
request, automatic retry, payment polling, webhook redesign or ERP sync is
added. CART-008 is remediated locally. See
[Order-owned Payment Retry](PAYMENT_RETRY.md).

Commerce Leasing Phase A is merged, MySQL CI verified, deployed and staging
schema/safety verified. Leasing discovery and
...
Public Cart and checkout routes remain disabled. CART-008 is remediated locally
by Phase 1D.2B and CART-023 remains open. Operational boundaries are documented in
