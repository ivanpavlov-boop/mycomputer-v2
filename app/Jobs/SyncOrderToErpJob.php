...
class SyncOrderToErpJob implements ShouldQueue
{
    ...
    public function handle(ErpService $erp): void
    {
        ...
        $order = Order::query()->with(['customer', 'user', 'items', 'bundleItems', 'paymentTransactions'])->findOrFail($job->entity_id);
        $adapter = $erp->provider($provider);

        if ($order->customer) {
            $adapter->pushCustomer($order->customer);
        } elseif ($order->user) {
            $adapter->pushCustomer($order->user);
        }

        $response = $adapter->pushOrder($order);
        $status = match (true) {
