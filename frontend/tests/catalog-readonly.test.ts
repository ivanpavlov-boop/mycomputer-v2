...
describe('read-only public catalog foundation', () => {
  ...
  it('hides shell commerce and account actions on read-only catalog routes', () => {
    ...
    expect(header).not.toContain('to="/account"')
    expect(header).not.toContain('to="/login"')
    expect(mobileMenu).toContain('storefrontPrimaryNavigation')
    expect(mobileMenu).not.toContain('to="/compare"')
    expect(mobileMenu).not.toContain('to="/account"')
    expect(mobileMenu).not.toContain('to="/login"')
