<template>
  <div v-if="page">
    <Breadcrumbs :items="[{ label: 'Ръководства', to: '/blog' }, { label: page.title }]" />
    ...
    <section v-if="page.related_categories?.length" class="container-page mt-10">
      <h2 class="section-title">Свързани категории</h2>
      <RelatedCategories class="mt-5" :categories="page.related_categories" />
    </section>
  </div>
</template>
...
const { data } = await useAsyncData(`seo-page-${slug}`, () => blog.seoPage(slug))
...
watchEffect(() => {
  ...
  if (page.value.schema_data) {
    useHead({ script: [{ type: 'application/ld+json', children: JSON.stringify(page.value.schema_data) }] })
  }
})
</script>
