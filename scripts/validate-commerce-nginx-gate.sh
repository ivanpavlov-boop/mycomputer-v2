...
validate_state() {
    ...
    docker run --rm -d \
        ...
        -e 'NGINX_ENVSUBST_FILTER=^(PUBLIC_COMMERCE_ENABLED|PUBLIC_COMMERCE_CONFIRMATION_ENABLED)$' \
...
validate_state closed false false 404 404 404
validate_state confirmation-only false true 404 404 200
validate_state open true true 200 200 200
validate_state invalid true false 404 404 404
